...
// Función de filtrado
function filtrar() {
    const categoriaSeleccionada = document.getElementById("categoria").value.trim().toLowerCase();
    const materialSeleccionado = document.getElementById("material").value.trim().toLowerCase();
    const productos = cargarProductos();

    // Normalizar texto eliminando acentos y espacios en blanco
    const normalizarTexto = (texto) => {
        return texto.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
    };

    const productosFiltrados = productos.filter(producto => {
        const categoriaProducto = normalizarTexto(producto.categoria);
        const materialProducto = normalizarTexto(producto.material);

        const coincideCategoria = categoriaSeleccionada === "todos" || categoriaProducto === categoriaSeleccionada;
        const coincideMaterial = materialSeleccionado === "todos" || materialProducto === materialSeleccionado;
...
function limpiarFiltro() {
    ...
    document.getElementById("material").value = "todos";
    const productos = cargarProductos();
    mostrarProductos(productos);
}

// Evento DOMContentLoaded para cargar productos iniciales y configurar botones
document.addEventListener("DOMContentLoaded", function () {
    const productos = cargarProductos();
    mostrarProductos(productos);

    // Configurar botones de filtrado y limpiar filtro
